...
#define LOG_TAG "ZmInternal-Offsets"
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, LOG_TAG, __VA_ARGS__)
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, LOG_TAG, __VA_ARGS__)

// Helper function to read string from memory (similar to ObterStr in legacy)
std::string ObterStr(uintptr_t address, uint32_t length) {
    if (!address || length == 0) {
        return "";
    }
    ...
    while (remaining > 0) {
        ...
        if (!Memory::ReadBuffer(currentAddr, buffer.data(), toRead)) {
            break;
        }
...
{
    uintptr_t ArrayBase = N32 ? Memory::Read<uint32_t>((uint32_t)this + 0x8) + 0x10 : Memory::Read<uint64_t>((uint64_t)this + 0x10) + 0x20;
    return ArrayBase;
}
...
{
    int Size = N32 ? Memory::Read<int>((uint32_t)this + 0xC) : Memory::Read<int>((uint64_t)this + 0x18);
...
{
    return Memory::Read<TValue>(GetItems() + (N32 ? 0x4 : 0x8) * Index);
}
...
uintptr_t Offsets::UnityDictionary<N32, V31, TValue>::GetValues()
{
    if constexpr (V31)
    {
        return N32 ? Memory::Read<uint32_t>((uint32_t)this + 0xC) + 0x10 : Memory::Read<uint64_t>((uint64_t)this + 0x18) + 0x20;
    }
    else
    {
        return N32 ? Memory::Read<uint32_t>((uint32_t)this + 0x14) + 0x10 : Memory::Read<uint64_t>((uint64_t)this + 0x28) + 0x20;
    }
}
...
int Offsets::UnityDictionary<N32, V31, TValue>::GetNumValues()
{
    ...
    if constexpr (V31)
    {
        Count = N32 ? Memory::Read<int>((uint32_t)this + 0x10) : Memory::Read<int>((uint64_t)this + 0x20);
    }
    else
    {
        Count = N32 ? Memory::Read<int>((uint32_t)this + 0x18) : Memory::Read<int>((uint64_t)this + 0x30);
    }
...
TValue Offsets::UnityDictionary<N32, V31, TValue>::GetValue(int Index)
{
    if constexpr (V31)
    {
        return Memory::Read<TValue>(GetValues() + (N32 ? 0x10 * Index + 0xC : 0x18 * Index + 0x10));
    }
    else
    {
        return Memory::Read<TValue>(GetValues() + (N32 ? 0x4 : 0x8) * Index);
    }
}
...
void Offsets::GameConfig()
{
    ...
    {
        LibIl2Cpp = candidate;
        LOGI("Testing libil2cpp.so candidate: 0x%llX", (uint64_t)candidate);

        // ==================== FF TH v7a 75 32-bit, OB54) ====================
        {
            uint32_t pTypeInfo = Memory::Read<uint32_t>(LibIl2Cpp + 0xABFF3B8);
            uint32_t StaticFields = Memory::Read<uint32_t>(pTypeInfo + 0x5C);
            uint32_t ReleaseVersion = Memory::Read<uint32_t>(StaticFields + 0x0);

            LOGI("[V7A] pTypeInfo    : 0x%X (from LibIl2Cpp+0xABFF3B8)", pTypeInfo);
            LOGI("[V7A] StaticFields : 0x%X (+0x5C)", StaticFields);
            LOGI("[V7A] ReleaseVersion: 0x%X", ReleaseVersion);

            if (ReleaseVersion)
            {
                std::string version = ObterStr(ReleaseVersion + 0xC, Memory::Read<uint32_t>(ReleaseVersion + 0x8));
                LOGI("[V7A] Release Version: %s", version.c_str());

                if (version == "OB54")
                {
                    // In a real implementation, we'd set g_Globals here
                    // For now, just log that we found the version
                    LOGI("[V7A] Found OB54 version!");
                    FFTHV7A75();
                    return;
                }
            }
        ...
        {
            uint32_t pTypeInfo = Memory::Read<uint32_t>(LibIl2Cpp + 0xABFF6D8);
            uint32_t StaticFields = Memory::Read<uint32_t>(pTypeInfo + 0x5C);
            uint32_t ReleaseVersion = Memory::Read<uint32_t>(StaticFields + 0x0);

            LOGI("[V7A] pTypeInfo    : 0x%X (from LibIl2Cpp+0xABFF6D8)", pTypeInfo);
            LOGI("[V7A] StaticFields : 0x%X (+0x5C)", StaticFields);
            LOGI("[V7A] ReleaseVersion: 0x%X", ReleaseVersion);
    ...
    LOGE("No matching version found!");
    LibIl2Cpp = 0;
}

void Offsets::FFTHV7A75() // v31 32-bit
{
    LOGI("Applying FFTHV7A75 offsets (v31 32-bit)");
...
{
    LOGI("Applying FFTHV7A76 offsets (v31 32-bit)");
