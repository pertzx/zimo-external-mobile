...
static float SmoothScroll(float current, float target, float deltaTime, float damping)
{
    float diff = target - current;
    return current + diff * ImClamp(deltaTime * damping, 0.0f, 1.0f);
}
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    bb.Expand(ImVec2(-ImClamp(IM_TRUNC((bb_frame.GetWidth() - 2.0f) * 0.5f), 0.0f, 3.0f),
        -ImClamp(IM_TRUNC((bb_frame.GetHeight()) * 0.5f), 0.0f, 3.0f)));
    ...
    ImRect click_area = bb;
    if (axis == ImGuiAxis_Y)
    {
        click_area.Min.x -= 0.5f;
        click_area.Max.x -= 0.5f;
    }
    ButtonBehavior(click_area, id, &hovered, &held, ImGuiButtonFlags_NoNavFocus);

    const ImS64 scroll_max = ImMax((ImS64)1, size_contents_v - size_avail_v);
    float scroll_ratio = ImSaturate((float)*p_scroll_v / (float)scroll_max);
    float grab_v_norm = scroll_ratio * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v;

    float scroll_v_norm = ImSaturate((float)*p_scroll_v / (float)scroll_max);

    if (held && allow_interaction && grab_v_norm < 1.0f)
    {
        ...
        const float new_scroll_v_norm = ImSaturate((clicked_v_norm - g.ScrollbarClickDeltaToGrabCenter - grab_h_norm * 0.5f) / (1.0f - grab_h_norm));
        *p_scroll_v = (ImS64)(new_scroll_v_norm * scroll_max);

        scroll_v_norm = ImSaturate((float)*p_scroll_v / (float)scroll_max);
        grab_v_norm = scroll_v_norm * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v;

        if (seek_absolute)
            g.ScrollbarClickDeltaToGrabCenter = clicked_v_norm - grab_v_norm - grab_h_norm * 0.5f;
    }

    float targetScroll = (float)(scroll_v_norm * scroll_max);
    *p_scroll_v = (ImS64)SmoothScroll((float)*p_scroll_v, targetScroll, g.IO.DeltaTime, 12.0f);
    ...
    if (axis == ImGuiAxis_X)
    {
        float smoothed = SmoothStep(0.0f, 1.0f, grab_v_norm);
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, smoothed), bb.Min.y,
            ImLerp(bb.Min.x, bb.Max.x, smoothed) + grab_h_pixels, bb.Max.y);
    }
    else
    {
        float smoothed = SmoothStep(0.0f, 1.0f, grab_v_norm);
        grab_rect = ImRect(bb.Min.x, ImLerp(bb.Min.y, bb.Max.y, smoothed),
            bb.Max.x, ImLerp(bb.Min.y, bb.Max.y, smoothed) + grab_h_pixels);
    }
    ...
    if (axis == ImGuiAxis_Y)
    {
        window->DrawList->AddRectFilledMultiColor(
            grab_rect.Min,
            grab_rect.Max,
            ImColor(200, 200, 200, (int)(GetStyle().Alpha * 0)),
            ImColor(200, 200, 200, (int)(GetStyle().Alpha * 0)),
            ImColor(200, 200, 200, (int)(GetStyle().Alpha * 200)),
            ImColor(200, 200, 200, (int)(GetStyle().Alpha * 255)));
    }
    return held;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int itemcount)
{
    ...
    const float preview_width = CalcTextSize(preview_value, nullptr, true).x;

    static std::vector<ComboAnim> Values;

    auto it = std::find_if(Values.begin(), Values.end(),
        [id](const ComboAnim& v) { return v.id == id; });

    if (it == Values.end())
    {
        Values.push_back({ id, 0.f, 0.f });
        it = std::prev(Values.end());
    }

    it->b = ImLerp(it->b, 19 + preview_width + 14, 0.1f);

    const ImRect bb(window->DC.CursorPos + ImVec2(GetWindowSize().x - 20 - it->b, 0),
        window->DC.CursorPos + ImVec2(GetWindowSize().x - 20, 25));
    ...
    it->a = ImLerp(it->a, popup_open ? 3.f : 2.f, 0.1f);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + total_bb.GetHeight() / 2),
            it->a, ImColor(255, 0, 255, (int)(GetStyle().Alpha * 255)), 360);
    }
    ...
    if (preview_value != nullptr && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(bb.Min + ImVec2(7, bb.GetHeight() / 2 - label_size.y / 2),
            bb.Max, preview_value, nullptr, nullptr);
    }

    if (label_size.x > 0)
        GetWindowDrawList()->AddText(total_bb.Min + ImVec2(0, total_bb.GetHeight() / 2 - label_size.y / 2),
...
{
    static std::vector<ComboPopupAnim> Values;

    auto it = std::find_if(Values.begin(), Values.end(),
        [popup_id](const ComboPopupAnim& v) { return v.id == popup_id; });

    if (it == Values.end())
    {
        Values.push_back({ popup_id, 0.f });
        it = std::prev(Values.end());
    }

    bool is_open = IsPopupOpen(popup_id, ImGuiPopupFlags_None);

    it->a = ImLerp(it->a, is_open ? 1.f : 0.f, 0.1f);
    ...
    float w = bb.GetWidth();
    SetNextWindowSize(ImVec2(w, (CalcMaxPopupHeightFromItemCount(itemcount) + 21) * it->a));
    ...
    PushStyleVar(ImGuiStyleVar_PopupRounding, 5);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data, bool IsIcon)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    static std::vector<input_state> anim;
    int current_frame = 0;
    current_frame++;

    if (current_frame % 500 == 0) {
        anim.erase(std::remove_if(anim.begin(), anim.end(),
            [current_frame](const input_state& state) {
                return current_frame - state.last_used_frame > 1000;
            }), anim.end());
    }

    auto it_anim = std::find_if(anim.begin(), anim.end(), [id](const input_state& state) { return state.id == id; });
    if (it_anim == anim.end())
    {
        input_state new_state;
        new_state.id = id;
        new_state.background = ImColor(30, 30, 36, (int)(GetStyle().Alpha * 255));
        new_state.border = ImColor(50, 50, 60, (int)(GetStyle().Alpha * 255));
        new_state.slow = -1.0f; // Valor especial indicando "não inicializado"
        new_state.last_used_frame = current_frame;
        anim.push_back(new_state);
        it_anim = anim.end() - 1;
    }
    else
    {
        it_anim->last_used_frame = current_frame;
    }

    // Cores do tema minimalista
    ImColor bgNormal = ImColor(30, 30, 30, (int)(GetStyle().Alpha * 255));
    ImColor bgHover = ImColor(35, 35, 35, (int)(GetStyle().Alpha * 255));
    ImColor bgActive = ImColor(38, 38, 38, (int)(GetStyle().Alpha * 255));
    ImColor borderNormal = ImColor(50, 50, 50, (int)(GetStyle().Alpha * 255));
    ImColor borderHover = ImColor(80, 80, 80, (int)(GetStyle().Alpha * 255));
    ImColor borderActive = ImColor(200, 0, 0, (int)(GetStyle().Alpha * 255)); // Pink accent
    
    ImColor targetBg = IsItemActive() ? bgActive : (hovered ? bgHover : bgNormal);
    ImColor targetBorder = IsItemActive() ? borderActive : (hovered ? borderHover : borderNormal);
    
    it_anim->background = ImLerp(it_anim->background, targetBg, g.IO.DeltaTime * 10.f);
    it_anim->border = ImLerp(it_anim->border, targetBorder, g.IO.DeltaTime * 8.f);

    if (!is_multiline)
    {
        // Background com bordas arredondadas
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->background), 6.f);
        // Borda sutil
        ImGui::GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->border), 6.0f, ImDrawFlags_RoundCornersAll, 1.0f);

        if (IsIcon) {
            ImGui::PushFont(Fonts::FontAwesomeSolid);
            GetWindowDrawList()->AddText(Fonts::FontAwesomeSolid, 15.0f, frame_bb.Max - ImVec2((size_arg.y + CalcTextSize(label).x) / 2, (size_arg.y + CalcTextSize(label).x) / 2 - 3), IM_COL32(255, 255, 255, 255), label);
            ImGui::PopFont();
        }
    }
    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(10, style.FramePadding.y);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            // Cor de seleção mais visível - pink accent com opacidade
            ImU32 bg_color = IM_COL32(255, 255, 255, 100);
            ...
            for (const ImWchar* p = text_selected_begin; p < text_selected_end; )
            {
                ...
                else
                {
                    ...
                    if (rect.Overlaps(clip_rect))
                        draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, 2.0f); // Bordas arredondadas
                }
        ...
        if (render_cursor)
        {
            ...
            // Animação suave do cursor (posição X)
            float targetCursorX = cursor_screen_pos.x;
            
            // Se é a primeira vez (slow < 0), definir diretamente sem interpolar
            if (it_anim->slow < 0.0f) {
                it_anim->slow = targetCursorX;
            } else {
                it_anim->slow = ImLerp(it_anim->slow, targetCursorX, g.IO.DeltaTime * 15.0f);
            }

            // Cor do cursor - pink accent
            ImU32 cursorColor = IM_COL32(255, 255, 255, cursor_is_visible ? 255 : 0);
            
            // Posição vertical do cursor centralizada
            float cursorTop = frame_bb.Min.y + style.FramePadding.y;
            float cursorBottom = frame_bb.Max.y - style.FramePadding.y;
            
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(ImVec2(it_anim->slow, cursorTop), ImVec2(it_anim->slow, cursorBottom), cursorColor, 1.5f);
