...
/*
 * g_FreeFireMemory agora é o Memory REAL (Client/Memory/Memory.cpp).
 * Todo READ/WRITE dele passa pela ponte: daemon root em
 * /data/local/tmp/stormdaemon (socket /data/local/tmp/stormbridge.sock).
 * O stub antigo do Android foi removido.
 */
...
{
        bIsMenuOpen = true;
        // REMOVIDO PRA PORTAR ANDROID: SetWindowLong(hWindow, GWL_EXSTYLE, WS_EX_TOPMOST | WS_EX_TOOLWINDOW | WS_EX_LAYERED);
        // REMOVIDO PRA PORTAR ANDROID: SetWindowPos(hWindow, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_FRAMECHANGED | SWP_SHOWWINDOW);
        // REMOVIDO PRA PORTAR ANDROID: SetForegroundWindow(hWindow);
}
                                                ...
                                                std::thread([&]()
                                                        {
                                                                ...
                                                                /*
                                                                 * AQUI a memória real entra: FindTargetPid + base de
                                                                 * libil2cpp via PONTE (daemon root). Se falhar,
                                                                 * avisamos mas o login continua - o watchdog do
                                                                 * Draw tenta de novo sozinho.
                                                                 */
                                                                if (!g_FreeFireMemory.Initialize())
                                                                {
                                                                        NotifyManager::Send(XorStr("Memoria: jogo nao encontrado (daemon rodando? jogo aberto?)"), 5000);
                                                                }
...
/* REMOVIDO PRA PORTAR ANDROID: void Interface::WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
        switch (uMsg)
        {
        case WM_SIZE:
