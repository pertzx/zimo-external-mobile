...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.b = ImLerp(it_anim->second.b, hovered ? ImColor(10, 145, 245, 200).Value : ImColor(ImGui::GetColorU32(ImGuiCol_Border)).Value, g.IO.DeltaTime * 10.f);
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    if (Enabled)
    {
        window->DrawList->AddRectFilled(total_bb.Min, total_bb.Max, ImColor(10, 145, 245, (int)(ImGui::GetStyle().Alpha * 255)), 5);

        ImGui::PushFont(FontAwesomeSolid);
        window->DrawList->AddText(total_bb.Min + ImVec2(15.f - ImGui::CalcTextSize(Icon).x / 2.f, total_bb.GetHeight() / 2.f - ImGui::CalcTextSize(Icon).y / 2.f), ImColor(1.f, 1.f, 1.f, ImGui::GetStyle().Alpha), Icon);
        ImGui::PopFont();

        window->DrawList->AddText(total_bb.Min + ImVec2(30.f, total_bb.GetHeight() / 2.f - ImGui::CalcTextSize(Label).y / 2.f), ImColor(1.f, 1.f, 1.f, ImGui::GetStyle().Alpha), Label);
    }
    else
    {
        ImGui::PushFont(FontAwesomeSolid);
        window->DrawList->AddText(total_bb.Min + ImVec2(15.f - ImGui::CalcTextSize(Icon).x / 2.f, total_bb.GetHeight() / 2.f - ImGui::CalcTextSize(Icon).y / 2.f), ImColor(1.f, 1.f, 1.f, 0.4f * ImGui::GetStyle().Alpha), Icon);
        ImGui::PopFont();

        window->DrawList->AddText(total_bb.Min + ImVec2(30.f, total_bb.GetHeight() / 2.f - ImGui::CalcTextSize(Label).y / 2.f), ImColor(1.f, 1.f, 1.f, 0.4f * ImGui::GetStyle().Alpha), Label);
    }
...
bool ImGui::SearchBar(const char* str_id, const ImVec2& size) {
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (effective_size.x == 0)
        effective_size.x = 200; // Default width if not specified
    if (effective_size.y == 0)
        effective_size.y = 35; // Match CustomChild height

    const ImRect bb(pos, pos + effective_size);

    // Register item for interaction (though non-functional)
    ImGui::ItemSize(bb, style.FramePadding.y);
    if (!ImGui::ItemAdd(bb, id))
        return false;

    // Draw background (same as CustomChild: dark gray with rounded corners)
    window->DrawList->AddRectFilled(
        pos,
        pos + effective_size,
        ImColor(16, 16, 16, (int)(style.Alpha * 255)),
        5.0f, // Same rounding as CustomChild
        ImDrawFlags_RoundCornersAll
    );

    // Draw border (same as CustomChild)
    ...
    // Push font for icon and placeholder text
    ImGui::PushFont(InterSemiBold);

    // Draw magnifying glass icon
    ImGui::PushFont(FontAwesomeSolid);
    ImVec2 icon_pos = pos + ImVec2(10, (effective_size.y - ImGui::CalcTextSize(ICON_FA_MAGNIFYING_GLASS).y) / 2);
    window->DrawList->AddText(
        icon_pos,
        ImColor(1.0f, 1.0f, 1.0f, 0.8f * style.Alpha),
        ICON_FA_MAGNIFYING_GLASS
    );
    ImGui::PopFont();

    // Draw placeholder text (e.g., "Search...")
    ImGui::PushFont(InterSemiBold);
    const char* placeholder = "Search here...";
    ImVec2 text_pos = pos + ImVec2(30, (effective_size.y - ImGui::CalcTextSize(placeholder).y) / 2);
    window->DrawList->AddText(
        text_pos,
        ImColor(1.0f, 1.0f, 1.0f, 0.5f * style.Alpha),
        placeholder
    );
    ImGui::PopFont();

    return false; // Non-functional, so return false (no interaction)
}
...
bool ImGui::TabHeaderButton(const char* ButtonName, bool enabled, int direction, int TabIndex)
{
    ...
    ImColor BGColor = enabled ? ImColor(10, 145, 245, (int)(GetStyle().Alpha * 255)) : ImColor(16, 16, 16, (int)(GetStyle().Alpha * 255));
    ...
    if (enabled)
        window->DrawList->AddRectFilled(ImVec2(window->Pos.x + Value->second.posx + 1, bb.Min.y + 1), ImVec2(window->Pos.x + Value->second.posx + size.x - 1, bb.Max.y - 1), ImColor(10, 145, 245, (int)(GetStyle().Alpha * 255)), 5, Corner);
...
bool ImGui::TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs, int TabIndex)
{
    ...
    SetCursorPos(ImVec2(20, 10) + ImVec2(bb.GetWidth() / 2 - WidhtMax / 2, 12.5f));
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_Y)
        window->DrawList->AddRectFilledMultiColor(grab_rect.Min, grab_rect.Max, ImColor(10, 145, 245, 0), ImColor(10, 145, 245, 0), ImColor(10, 145, 245, (int)(GetStyle().Alpha * 255)), ImColor(10, 145, 245, (int)(GetStyle().Alpha * 255)), 5);
...
static void RenderPinkToggle(ImGuiWindow* window, const ImRect& total_bb, float animation, bool hovered)
{
    const float alpha = ImGui::GetStyle().Alpha;
    const float enabled = 1.0f - animation;
    const ImVec2 toggle_size(36.0f, 20.0f);
    const ImVec2 toggle_min(
        total_bb.Max.x - toggle_size.x,
        total_bb.Min.y + (total_bb.GetHeight() - toggle_size.y) * 0.5f
    );
    const ImVec2 toggle_max = toggle_min + toggle_size;

    const ImU32 off_color = ImColor(31, 31, 35, static_cast<int>(255.0f * alpha));
    const ImU32 hover_color = ImColor(43, 43, 48, static_cast<int>(255.0f * alpha));
    const ImU32 on_color = ImColor(10, 145, 245, static_cast<int>(255.0f * alpha));
    const ImU32 border_color = ImColor(66, 66, 72, static_cast<int>(255.0f * alpha));

    window->DrawList->AddRectFilled(
        toggle_min,
        toggle_max,
        enabled > 0.5f ? on_color : (hovered ? hover_color : off_color),
        toggle_size.y * 0.5f
    );
    window->DrawList->AddRect(toggle_min, toggle_max, border_color, toggle_size.y * 0.5f, 0, 1.0f);

    const float knob_x = toggle_min.x + 10.0f + enabled * (toggle_size.x - 20.0f);
    const ImVec2 knob_center(knob_x, toggle_min.y + toggle_size.y * 0.5f);
    window->DrawList->AddCircleFilled(
        knob_center,
        6.5f,
        enabled > 0.5f
            ? ImColor(255, 255, 255, static_cast<int>(255.0f * alpha))
            : ImColor(166, 166, 172, static_cast<int>(255.0f * alpha)),
        24
    );
}
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    RenderPinkToggle(window, total_bb, Value->second.a, hovered);
...
bool ImGui::CheckboxRisk(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    // Desenha o "( Risk )" em vermelho logo ap�s o label
    ImVec2 risk_pos = ImVec2(total_bb.Min.x + label_size.x + 8, total_bb.Min.y + total_bb.GetHeight() / 2 - label_size.y / 2);
    window->DrawList->AddText(risk_pos, IM_COL32(255, 0, 0, 255), "( Risk )");
...
bool ImGui::KeyBind(const char* label, int* Key, int* KeyState)
{
    ...
    else if (io.MouseClicked[0] || io.MouseClicked[1])
    {
        if (g.ActiveId == id)
            ClearActiveID();
    }
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int itemcount)
{
    ...
    if (!(flags & ImGuiComboFlags_NoPreview))
    {
        ...
        window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + total_bb.GetHeight() / 2), Value->second.a, ImColor(10, 145, 245, (int)(GetStyle().Alpha * 255)), 360);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 0), ImVec2(frame_bb.Min.x + Value->second.anim + 1, frame_bb.Max.y), ImColor(10, 145, 245, (int)(GetStyle().Alpha * 255)), 5);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    it_anim->second.border = ImLerp(it_anim->second.border, IsItemActive() || hovered ? ImColor(96, 96, 102, 255) : ImColor(30, 30, 30), g.IO.DeltaTime * 3.f);
...
static ImGuiTabItem* ImGui::TabBarTabListPopupButton(ImGuiTabBar* tab_bar)
{
    ...
    PushStyleColor(ImGuiCol_Text, arrow_col);
    PushStyleColor(ImGuiCol_Button, ImVec4(0, 0, 0, 0));
    bool open = BeginCombo("##v", NULL, ImGuiComboFlags_NoPreview | ImGuiComboFlags_HeightLargest, NULL);
    PopStyleColor(2);
