...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    if (g.IO.KeyShift)
        for (int merge_group_n = 0; merge_group_n < IM_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            ...
            GetForegroundDrawList()->AddText(text_pos, IM_COL32(255, 255, 0, 0), buf, NULL);
            GetForegroundDrawList()->AddRect(merge_group->ClipRect.Min, merge_group->ClipRect.Max, IM_COL32(255, 255, 0, 0));
        }
#endif
    ...
    for (int pass = 0; pass < 2; pass++)
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (pass == 0)
            {
                ...
                //if (g.IO.KeyShift) { draw_list->AddRect(label_r.Min, label_r.Max, IM_COL32(0, 255, 0, 0), 0.0f, 0, 2.0f); }
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    if (IsItemHovered())
        GetForegroundDrawList()->AddRect(table->OuterRect.Min, table->OuterRect.Max, IM_COL32(255, 255, 0, 0));
    if (IsItemVisible() && table->HoveredColumnBody != -1)
        GetForegroundDrawList()->AddRect(GetItemRectMin(), GetItemRectMax(), IM_COL32(255, 255, 0, 0));
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        if (IsItemHovered())
        {
            ...
            GetForegroundDrawList()->AddRect(r.Min, r.Max, IM_COL32(255, 255, 0, 0));
        }
    }
